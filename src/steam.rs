...
fn dlss_cache_path(app_id: u32) -> PathBuf {
    dlss_cache_dir().join(format!("{}.txt", app_id))
}

fn load_cached_dlss_path(app_id: u32) -> Option<PathBuf> {
    let path = std::fs::read_to_string(dlss_cache_path(app_id)).ok()?;
...
fn store_cached_dlss_path(app_id: u32, dll_path: &Path) {
    let _ = std::fs::write(dlss_cache_path(app_id), dll_path.to_string_lossy().as_bytes());
}
...
#[cfg(target_os = "windows")]
fn detect_dlss_version(install_path: &Path, app_id: Option<u32>) -> Option<String> {
    ...
    if let Some(app_id) = app_id {
        if let Some(cached_path) = load_cached_dlss_path(app_id) {
    ...
    if let Some(app_id) = app_id {
        store_cached_dlss_path(app_id, &dll_path);
    }
...
fn detect_dlss_version(_install_path: &Path, _app_id: Option<u32>) -> Option<String> {
    None
}
            ...
            for entry in entries.filter_map(|e| e.ok()) {
                ...
                if let Ok(content) = std::fs::read_to_string(&path) {
                    ...
                    if let Some(id) = app_id {
                        ...
                        let dlss_version = detect_dlss_version(&game_path, Some(id));
                        games.push(Game {
                            name,
                            path: game_path,
                            app_id: Some(id),
                ...
                for subkey_name in uninstall_key.enum_keys().filter_map(|k| k.ok()) {
                    if let Some(caps) = steam_app_re.captures(&subkey_name) {
                        ...
                        if let Ok(subkey) = uninstall_key.open_subkey(&subkey_name) {
                            ...
                            let dlss_version = detect_dlss_version(&install_path, Some(app_id));
